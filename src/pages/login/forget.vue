<template>
    <div class="container">
        <header class="header">忘记密码</header>
         <div class="content">
             <van-field v-model="tel"   placeholder="请输入手机号" :error-message="usertel"  clearable />

             <van-field   v-model="sms"   center clearable   placeholder="请输入短信验证码" :error-message="test">
             <van-button :disabled="flag" slot="button" size="small" type="primary" @click="sendCode">{{ buttonmsg }}</van-button>
             </van-field>

             <van-field v-model="password" type="password"   placeholder="请重新输入密码" :error-message="pass" clearable />

              <van-button type="primary" :loading="loading" loading-text="保存中..." size="large" :disabled="zhud" @click="register">保存并登录</van-button>
              <van-divider @click="toLogin">注册</van-divider>
         </div>
    </div>
</template>
<script>
